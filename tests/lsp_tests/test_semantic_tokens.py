...
HOVER_TEST_ROOT = TEST_DATA / "semantic_tokens"

# pylint: disable=line-too-long


def test_semantic_tokens_import():
    ...
    with session.LspSession() as ls_session:
        ...
        uri = as_uri(HOVER_TEST_ROOT / "semantic_tokens_test1.py")
        actual = ls_session.text_document_semantic_tokens_full(
            {
...
def test_semantic_tokens_import_from():
    ...
    with session.LspSession() as ls_session:
        ls_session.initialize()
        uri = as_uri(HOVER_TEST_ROOT / "semantic_tokens_test2.py")
        actual = ls_session.text_document_semantic_tokens_full(
            {
                "textDocument": {"uri": uri},
            }
